...
### Fixed
- Fixed race condition causing phone number to disappear on blur in MudIntlTelInput
- Fixed `SetTextAsync` to synchronously update `Text` property, preventing value loss during quick blur events

## [3.0.5] - 2026-01-30

### Fixed
- Fixed caching issues with versions 3.0.1 to 3.0.4

## [3.0.4] - 2026-01-30

### Fixed
- Fixed `UtilsScript` default path from relative (`./`) to absolute path to ensure proper loading in all hosting scenarios

## [3.0.3] - 2026-01-27

### Fixed
- Fixed JS interop module path from relative (`./`) to absolute (`/`) to ensure proper loading in all hosting scenarios

## [3.0.2] - 2026-01-26

### Fixed
- Fixed `OnTextAsync` to simply return Task.CompletedTask so applications stay responsive
- Fixed `_inputIndex` initialization to `-1` to properly track uninitialized state
